...
1. **线程安全**：确保多线程环境下的安全性
   - 使用 Qt 的跨线程调用机制（QMetaObject::invokeMethod 或信号槽连接）实现公开方法的调用
   - 所有公开方法（start、stop、disconnect、setInterval、state）通过信号槽机制或 invokeMethod 调用，确保在对象所属线程中执行
   - 内部状态更新和 QSerialPort 操作均在主线程（对象创建线程）中执行，避免多线程竞争
   - 状态枚举、连接模式等成员变量的读写通过互斥锁保护（如需要）
