...
public:
    ...
    // 设置当前日志文件名（配合 debug/info/warn/error 便捷方法使用）
...
public:
    ...
    // 6.1.2 6.1.2 6.1.2 6.1.2 6.1.2 6.1.2 6.1.2 便捷日志方法：使用 set_current_log_file() 设置的当前文件名
    template<typename... Args>
    void debug(const std::string& fmt, Args&&... args) {
        log(get_current_log_file(), Level::DEBUG, fmt, std::forward<Args>(args)...);
    }

    template<typename... Args>
    void info(const std::string& fmt, Args&&... args) {
        log(get_current_log_file(), Level::INFO, fmt, std::forward<Args>(args)...);
    }

    template<typename... Args>
    void warn(const std::string& fmt, Args&&... args) {
        log(get_current_log_file(), Level::WARN, fmt, std::forward<Args>(args)...);
    }

    template<typename... Args>
    void error(const std::string& fmt, Args&&... args) {
        log(get_current_log_file(), Level::ERROR, fmt, std::forward<Args>(args)...);
    }
...
private:
    ...
    std::string current_log_file_;              // 当前日志文件名（用于便捷方法）
